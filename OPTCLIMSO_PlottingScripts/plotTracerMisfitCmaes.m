function plotTracerMisfitCmaes(optDir, outDir)
...
%
% Examples: 
% plotTracerMisfit('/Users/orie3677/Documents/Uni/Project/Optimization/OptClim/OxfordMOPS/test_MOPS_cmaes')
% plotTracerMisfit('/Users/orie3677/Documents/Uni/Project/Optimization/OptClim/OxfordMOPS/test_MOPS_cmaes', '/Users/outFigures')
%
...
% Read JSON file
[~, ~, allMisfits, ~] = i_readFinalJSON(optDir);

% Get Generation Info
genFields = fieldnames(allMisfits);
genNum = cellfun(@(x) str2double(x(end-2:end)), genFields);
maxGen = max(genNum);
[~,iOrder] = sort(genNum);

% Get CMAES Results from Folders (not from JSON file!!)
[misfits, ~, bestPop, bestGen] = i_getCmaesResults(optDir, genFields(iOrder), []);

%% Misfits

% Get Total Misfit Info
[misNames, tracers, numReg] = i_getMisfitsInfo(allMisfits.(genFields{1}));

% Get Cost Values
x = 1:maxGen;
genSize = length(misfits(1,:));
popSize = length(misfits(:,1));
yBest = NaN(1,genSize);
yBad = NaN(popSize-1, genSize);
for b = 1:genSize
    igood = 1:popSize == bestPop(b);
    yBest(b) = misfits(igood,b);
    yBad(:,b) = misfits(~igood, b);
end
...
for t = 1:numel(tracers)
    ...
    for g = 1:maxGen
        ig = genNum == g;
        thisGen = allMisfits.(genFields{ig});
...
plot(axp(1), x, log(yBest), 'k-')
hold(axp(1), 'on')
plot(axp(1), x, log(yBad), 'k.')

% Plot best
i_plotBest(axp(1), bestGen, get(axp(1), 'ylim'))
bestCost = yBest(bestGen);
...
for t = 1:nt
    p(t) = plot(axp(2), x, fracTrMis{t}, [c{t}, '-']);
end
