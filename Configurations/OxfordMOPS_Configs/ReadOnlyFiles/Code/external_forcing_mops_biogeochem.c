...
/* Read and overwrite default parameter values here */
  ...
  if (readBGCParams) {
    ierr = PetscOptionsGetString(PETSC_NULL,"-num_bgc_params_file",numBGCParamsFile,PETSC_MAX_PATH_LEN-1,&flg);CHKERRQ(ierr);
    ...
    ierr = PetscOptionsGetString(PETSC_NULL,"-bgc_paramnames_file",bgcNamesFile,PETSC_MAX_PATH_LEN-1,&flg);CHKERRQ(ierr);
...
/* IK : added for misfit function : start */
      ...
      if (doMisfit) {
		if (Iter0+iLoop>=costTimer.startTimeStep) { /* start time averaging (note: startTimeStep is ABSOLUTE time step) */  
		  ierr = VecSetValues(modbgc[0],lSize,gIndices,localmbgc1,INSERT_VALUES);CHKERRQ(ierr);
		  ierr = VecAssemblyBegin(modbgc[0]);CHKERRQ(ierr);
		  ierr = VecAssemblyEnd(modbgc[0]);CHKERRQ(ierr);    
  
		  ierr = VecSetValues(modbgc[1],lSize,gIndices,localmbgc2,INSERT_VALUES);CHKERRQ(ierr);
		  ierr = VecAssemblyBegin(modbgc[1]);CHKERRQ(ierr);
		  ierr = VecAssemblyEnd(modbgc[1]);CHKERRQ(ierr);    
  
		  ierr = VecSetValues(modbgc[2],lSize,gIndices,localmbgc3,INSERT_VALUES);CHKERRQ(ierr);
		  ierr = VecAssemblyBegin(modbgc[2]);CHKERRQ(ierr);
		  ierr = VecAssemblyEnd(modbgc[2]);CHKERRQ(ierr);    
        }
	  }
