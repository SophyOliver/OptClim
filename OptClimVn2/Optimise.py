...
version='$Id: Optimise.py 664 2018-04-26 11:56:43Z stett2 $' # subversion magic
revision='$Rev: 664 $'
...
def rangeAwarePerturbations(baseVals, parLimits, steps, nrandom,
...
# get constants from JSON's directory giving them a default value if not present.
    ...
    reg_crit_cond = get_default(studyJSON, 'reg_crit_covar',10e10) # condition number above which we regularise hessian
...
## NB this will change the results so do after all tests passed.
    pwr=np.floor(np.log10(np.fabs(param_value[0,:]))) # compute power of 10 for params
    param_scale = np.ones( (nParam) ) # default scaling is one for all parameters
    indx= (pwr < 0) & (np.fabs(param_value[0,:]) > 1e-10) # find those parameters that need to be scaled up because they are less than 1 and are not tiny!
...
# Deal with constraint
    if sigma:
       ...
       if constraint.ndim == 2:
           ...
           use_UM_value=np.hstack([use_UM_value, constraint[:,np.newaxis]]) # add constraint value to end of UM value
...
def run_fn(function, params, npt, constraint_target=None):
    ...
    values=[]
    for k in xrange(0, params.shape[0]):
