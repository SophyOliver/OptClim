...
from OptClimVn2.Optimise import doGaussNewton,calcErr,doLineSearch, doDFBOLS, randSelect, gaussNewton
...
# work out synthetic observations
        self.constraint=np.zeros(self.nparam+1) ; self.constraint[self.nparam]=0.5 # set up constraint
        ...
        tlst=[]
        s_vector=self.step*np.hstack((2*np.ones(self.nobs),1))/np.hstack((signs,0.5)) # how we perturb the parameters to get to zero error.
...
# all base params are greater than mid-point so perturbations move towards zero.
    ...
    def run_LineSearch(self, ref=False,trace=False):
        ...
        if ref:
            ...
            self.studyJSON['covIV']=self.LScov_iv.copy() # hack as reference code expects this to be in the options dict passed in...
            ...
            status,err,err_constraint,NewParam, index, bestParam,info=doLineSearch(
                ...
                olist= self.olist,
                constraint=self.LSconstraint, constraint_target= self.LSconstraint_target, studyJSON=self.studyJSON,trace=trace)
    ...
    def test_val_eq_tgt(self):
        ...
        for i in range(result['linesearch'].shape[0]):
            nptest.assert_allclose(result['linesearch'][i,:],self.param_value[0,:],err_msg='expected all LS to be same as initial')
    ...
    def test_all_same(self):
        ...
        for i in range(1,result['linesearch'].shape[0]):
            nptest.assert_allclose(result['linesearch'][i,:],result['linesearch'][0,:],err_msg='expected all param vectors to be identical')
...
# need to get rid of the constraint value as we are keeping that out.. 
    ...
    def test_randSelect(self):
        ...
        self.assertEquals(len(indx), nrandom+1)
