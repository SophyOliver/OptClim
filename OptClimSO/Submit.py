"""
Functions for model submission. Idea is one function per machine/cluster/os.
Currently defined are:
    1) submit -- which doesn't actually submit anything but runs fakemodel on each model to generate observations
    2) eddieSubmit - which submits models (and ancillary scripts) to Eddie.
"""
import os
import netCDF4
import subprocess
...
def submit(model_list, config, rootDir, verbose=False, resubmit=None, runCode=None):
    """
    ...
    for model in model_list:
        ...
        obs.name=model.name()
        # netCDF4 implementation...
        rootgrp = netCDF4.Dataset(outPath, "w", format="NETCDF4")
